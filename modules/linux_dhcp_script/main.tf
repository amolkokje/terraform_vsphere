...
resource "vsphere_virtual_machine" "linux-vm" {
    ...
    provisioner "remote-exec" {
		...
		inline = [
            ...
            # execute the script
            "bash /tmp/${var.vm_customization_script} --dns1 ${ element( split(",", var.vm_dns_servers), 0) } --dns2 ${ element( split(",", var.vm_dns_servers), 1) } --domain ${var.vm_domain} --hostname ${var.vm_name}"
		]
	}
}
